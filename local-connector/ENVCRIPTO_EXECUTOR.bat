...
if exist "%~dp0INVCRIPTO.ico" if not exist "%~dp0CLIQUE AQUI - ENVCRIPTO.lnk" (
  powershell -NoProfile -ExecutionPolicy Bypass -Command "$s=(New-Object -ComObject WScript.Shell).CreateShortcut('%~dp0CLIQUE AQUI - ENVCRIPTO.lnk'); $s.TargetPath='%~dp0ENVCRIPTO_EXECUTOR.bat'; $s.WorkingDirectory='%~dp0'; $s.IconLocation='%~dp0INVCRIPTO.ico'; $s.Save()" >nul 2>nul
)
