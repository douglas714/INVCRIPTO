...
export async function handler(event) {
  ...
  if (!credential) return json(404, { error: 'Nenhuma API salva para atualizar. Salve a API uma vez primeiro.' });
  ...
  const { data: command, error: commandError } = await supabase
    ...
    .insert({
      ...
      payload: {
        ...
        reason: 'Atualizacao manual de saldo solicitada no painel'
      },
      status: 'pending'
    })
    .select('id')
