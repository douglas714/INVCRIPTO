...
export function runPaperDecision(state, candles, usdtBrl=5.2) {
  ...
  if (analysis.action === 'BUY' && analysis.score >= 78 && next.positions.length === 0) {
    const valueBrl = Math.max(10, next.balanceBrl * 0.10);
    ...
    if (grossBrl >= target) {
      ...
      next.positions = [];
      next.decisions.unshift({ at: now, symbol: state.symbol, action:'WAIT', score:0, reason:'Operacao fechada com lucro. Reanalisando mercado e aguardando retracao.', regime:analysis.regime, support:analysis.support, resistance:analysis.resistance, price });
