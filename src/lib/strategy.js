...
export function analyzeMarket(candles) {
  ...
  const distanceFromResistance = resistance > 0 ? (resistance - last.close) / resistance : 0;
  const tooCloseToResistance = distanceFromResistance < 0.004;
  let score = 0, action = 'WAIT', reason = 'Sem setup';
  if ((pullbackBuy || (trendUp && rejectionBuy)) && !tooCloseToResistance) {
    score = 72 + (rejectionBuy ? 12 : 0); action='BUY'; reason = rejectionBuy ? 'Alta + varredura de suporte' : 'Alta + pullback EMA';
  } else if ((pullbackBuy || (trendUp && rejectionBuy)) && tooCloseToResistance) {
    score = 0; action='WAIT'; reason = 'Compra bloqueada: preco muito perto da resistencia';
  }
