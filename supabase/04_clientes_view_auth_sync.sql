...
begin
  ...
  values (
    new.id,
    coalesce(new.email, ''),
    coalesce(new.raw_user_meta_data->>'full_name', split_part(coalesce(new.email, 'Usuario'), '@', 1), 'Usuario'),
    coalesce(new.raw_user_meta_data->>'phone', 'nao informado'),
    'client',
    'active'
  )
...
select
  u.id,
  coalesce(u.email, ''),
  coalesce(u.raw_user_meta_data->>'full_name', split_part(coalesce(u.email, 'Usuario'), '@', 1), 'Usuario'),
  coalesce(u.raw_user_meta_data->>'phone', 'nao informado'),
  'client',
  'active'
from auth.users u
