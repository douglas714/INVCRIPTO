...
create table if not exists public.profit_events (
  ...
  fee_percent numeric(8,4) not null default 25,
...
('inv', '{"initial_bonus":10,"fee_percent":25,"brl_per_inv":1}'::jsonb),
...
-- Patch cadastro 2026-06-15: necessario para novos usuarios em producao
...
begin
  ...
  values (new.id, coalesce(new.email, ''), v_name, v_phone, 'client', 'active')
  on conflict (id) do update set
    email = excluded.email,
    full_name = coalesce(nullif(excluded.full_name, ''), public.profiles.full_name),
  ...
  if v_cpf_hash is not null then
    ...
    on conflict (user_id) do update set cpf_hash = excluded.cpf_hash, cpf_masked = excluded.cpf_masked;
  end if;
